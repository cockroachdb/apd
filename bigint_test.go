...
// TestBigIntBitsNotExposed tests that the BigInt wrapper does not expose access
// to the underlying word slice, unlike big.Int.
func TestBigIntBitsNotExposed(t *testing.T) {
	panics := func(fn func()) (saw bool) {
		defer func() {
			if r := recover(); r != nil {
				saw = true
			}
	...
	var b BigInt
	if !panics(func() { b.Bits() }) {
		t.Errorf("expected (*BigInt).Bits to panic")
	}
	if !panics(func() { b.SetBits([]big.Word{1, 2}) }) {
		t.Errorf("expected (*BigInt).SetBits to panic")
	}
}
